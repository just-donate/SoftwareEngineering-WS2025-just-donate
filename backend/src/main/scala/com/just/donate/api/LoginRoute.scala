...
object LoginRoute {
  ...
  def loginRoute: (Config, Repository[String, User]) => HttpRoutes[IO] = (appConfig: Config, userRepo: Repository[String, User]) =>
    HttpRoutes.of[IO]:
  ...
  private def authenticate(login: LoginRequest, appConfig: Config, userRepo: Repository[String, User]): IO[Response[IO]] = {
