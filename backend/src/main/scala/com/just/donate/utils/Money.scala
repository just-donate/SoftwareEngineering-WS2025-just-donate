...
object Money:
  ...
  def moneyStringTransform(input: String): String =
    ...
    val noSignStr = if negative then trimmed.drop(1) else trimmed
    ...
    val integerPart = if parts.nonEmpty then parts(0) else ""
    val fractionPart = if parts.length == 2 then parts(1) else ""

    // Step 5: Strip leading zeros from integer part
    val strippedInt = integerPart.replaceFirst("^0+", "")
    val finalInt = if strippedInt.isEmpty then "0" else strippedInt

    // Step 6: Strip trailing zeros from fraction part
    val strippedFrac = fractionPart.replaceAll("0+$", "")
    val finalFrac = if strippedFrac.isEmpty then "0" else strippedFrac
