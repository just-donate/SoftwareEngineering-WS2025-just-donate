...
public class Account {
    ...
    private BigDecimal getUpstreamBalance(String earmarking) {
        BigDecimal balance = getBoundQueue(earmarking).totalBalance();
    ...
    private DonationQueue getBoundQueue(String earmarking) {
        return boundDonations.stream()
                .filter(bound -> bound._1.equals(earmarking))
                .map(Tuple2::_2)
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Earmarking not found"));
    }
