...
export const DonationItem: React.FC<DonationItemProps> = ({
  ...
  useEffect(() => {
    ...
    if (!shownDonationIds.includes(donation.donationId)) {
      ...
      Cookies.set('shownDonations', JSON.stringify(updatedShownDonations), {
        expires: 365,
      });
  ...
  return (
    ...
    >
      {showThankYou && (
        <div
          className={`absolute rounded-lg inset-0 flex items-center justify-center bg-white bg-opacity-90 z-10 transition-opacity duration-500 ${isExiting ? 'opacity-0' : 'opacity-100'}`}
        >
          <div
            className={`text-center transition-transform duration-500 ${isExiting ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
          >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div className="flex-grow">
          <div className="flex items-center justify-between sm:justify-start">
            <h3 className={`text-xl font-semibold ${theme.text} mr-4`}>
